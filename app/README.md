...
| ChatGPT (Codex) | `codex` | `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-luna`, … |

Codex has no "list models" command, so its list is read out of the shipped
binary's strings and curated. Whatever `~/.codex/config.toml` says always
leads the list and is the default, so a model newer than this repo stays
reachable by editing that file. Claude's aliases resolve to the current
